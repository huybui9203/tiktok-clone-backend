...
const generateToken = (
    userId,
    role,
    expires = JWT_EXPIRES.accessToken,
    secret = env.JWT_SECRET_AT
) => {
    const payload = {
        sub: userId,
        role,
...
const generateAuthTokens = async (
    userId,
    role = 'user',
    ATExpires = JWT_EXPIRES.accessToken,
    RTExpires = JWT_EXPIRES.refreshToken
) => {
    try {
...
const verifyToken = (token, type) => {
    try {
        const jwtSecret =
            type === jwtTypes.ACCESS ? env.JWT_SECRET_AT : env.JWT_SECRET_RT;
